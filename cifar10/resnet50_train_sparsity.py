...
"""A binary to train RESNET-50 using a single GPU.
...
tf.app.flags.DEFINE_string('train_dir', '/tmp/resnet50_train',
...
tf.app.flags.DEFINE_string('sparsity_dir', '/tmp/resnet50_sparsity',
...
def train():
  ...
  with tf.Graph().as_default() as g:
    ...
    with tf.device('/cpu:0'):
      images, labels = resnet50.distorted_inputs()

    # Build a Graph that computes the logits predictions from the
    # inference model.
    logits, tensor_list = resnet50.inference(images)

    # Calculate loss.
    loss = resnet50.loss(logits, labels)

    # Build a Graph that trains the model with one batch of examples and
    # updates the model parameters.
    train_op, retrieve_list = resnet50.train(loss, tensor_list, global_step)
...
def main(argv=None):  # pylint: disable=unused-argument
  resnet50.maybe_download_and_extract()
